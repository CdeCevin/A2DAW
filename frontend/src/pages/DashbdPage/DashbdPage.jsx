...
import { Card, Table, EmptyState, Skeleton, Spinner } from "@heroui/react";
import { Users, PawPrint, CalendarDays, ClipboardList } from "lucide-react";
import { dashbdApi } from "../../api/dasbdApi";
import { useErrorHandler } from "../../api/errorHandler";

const links = [
    { desc: "Agendar Cita", icon: CalendarDays, lnk: "/citas", role: "public" },
    { desc: "Ver Mascotas", icon: PawPrint, lnk: "/mascotas", role: "public" },
    { desc: "Ver Dueños", icon: Users, color: "text-azul-vg", lnk: "/duenos", role: "public" },
    { desc: "Tratamientos", icon: ClipboardList, lnk: "/tratamientos", role: "admin" },
]
...
export default function DashbdPage() {
    ...
    const cards = [
        { desc: "Total Dueños", icon: Users, color: "text-azul-vg", colorfondo: "bg-azul-vg/20", dataKey: "totalDuenos" },
        { desc: "Total Mascotas", icon: PawPrint, color: "text-aqua-vg", colorfondo: "bg-aqua-vg/20", dataKey: "totalMascotas" },
        { desc: isAdmin ? "Citas Recientes" : "Citas Próximas", icon: CalendarDays, color: "text-accent-morado-vg", colorfondo: "bg-accent-morado-vg/20", dataKey: "totalCitas" },
        { desc: "Tratamientos", icon: ClipboardList, color: "text-accent-naranjo-vg", colorfondo: "bg-accent-naranjo-vg/20", dataKey: "totalTratamientos" },
    ]
            ...
            <div className="pt-20 sm:pt-24 md:pt-28 px-4 sm:px-6 lg:px-8 max-w-8xl mx-auto min-h-screen flex flex-col pb-6">
                ...
                <div className="flex flex-col gap-4 flex-1">
                    ...
                    <div className="grid grid-cols-2 lg:flex lg:flex-row gap-4 justify-evenly max-w-screen">
                        {(isAdmin ? cards : cards.slice(0, 3)).map((card, key) => (
                            isLoading ? (
                                <Skeleton 
                                    key={key} 
                                    className="w-full max-w-md h-26 rounded-xl" 
                                />
                            ) : (
                                ...
                                <Card.Content>
                                    <div className="flex flex-row items-center gap-5">
                                        ...
                                        <div className="flex flex-col w-full">
                                                <span className="font-bold text-2xl">{datos?.[card.dataKey] ?? "0"}</span>
                            ...
                            <Card.Content className="flex-1 overflow-auto p-0">
                                <Table className="bg-white h-full min-h-full">
                                    <Table.ScrollContainer>
                                        <Table.Content aria-label="Custom styled table" selectionMode="single" onSelectionChange={() => { navigate("/citas") }}>
                                            ...
                                            <Table.Body 
                                                renderEmptyState={() => (
                                                    isLoading ? (
                                                    <div className="flex flex-row items-center gap-1 w-full justify-center mt-2">
                                            ...
                                            >
                                                {!isLoading && (isAdmin ? datos?.citasRecientes : datos?.citasProximas)?.map((data, key) => (
                                                    <Table.Row key={key} className="border-b cursor-pointer hover:bg-gray-100 transition-colors">
                                                        ...
                                                        <Table.Cell className="text-slate-600">{data.motivo || "-"}</Table.Cell>
                                                        <Table.Cell>{"Dr. " + data.veterinario.name}</Table.Cell>
                                                        <Table.Cell className="text-slate-600">{data.diagnostico || "-"}</Table.Cell>
                                                    </Table.Row>
                                                ))}
                                            </Table.Body>
                                        </Table.Content>
                        ...
                        <Card className="flex flex-col w-full lg:w-80 bg-white border">
                            ...
                            <Card.Content className="flex-1 overflow-auto">
                                <div className="flex flex-col gap-2">
                                    {links
                                        .filter((link) => (link.role === "admin" ? isAdmin : true))
                                        .map((link, key) => (
